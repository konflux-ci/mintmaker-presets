...
- **Validate**: Use the following Podman command locally to validate a config preset (replace `<filename.json>` with the file you are editing) and fix any reported issues: 
  ...
  Requires Podman and network access to pull `quay.io/konflux-ci/mintmaker-renovate-image:latest`.
...
- **Scope**: Only change preset JSON and docs unless asked.
